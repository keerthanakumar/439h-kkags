...
void thread_init(void) {
	...
	for (i = NTHREAD-1; i >= 0; i--) {
 		thrds[i].thread_id = 0;
		thrds[i].thread_status = ENV_FREE;
		thrds[i].thread_runs = 0;
		thrds[i].thread_link = env_free_list;
...
int thread_alloc(struct Thread** newthread_store/*, threadid_t parent_id*/) { //the commented part is for matching what's in kern/env.c
	...
	t->thread_type = THREAD_TYPE_BABY;
...
void thread_create(uint8_t* binary, size_t, enum EnvType type) {
	struct Thread* t;
